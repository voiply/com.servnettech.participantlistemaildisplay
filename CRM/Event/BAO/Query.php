...
class CRM_Event_BAO_Query {
  ...
  public static function select(&$query) {
    if (($query->_mode & CRM_Contact_BAO_Query::MODE_EVENT) ||
      ...
      //email - added by Servnet
      if(CRM_Utils_Array::value('participant_email', $query->_returnProperties)){
        $query->_select['participant_email'] = 'civicrm_email.email as participant_email';
        $query->_element['participant_email'] = 1;
        $query->_tables['civicrm_email'] = 1;
        $query->_whereTables['civicrm_email'] = 1;
      }
    }
  }
  ...
  public static function whereClauseSingle(&$values, &$query) {
    ...
    switch ($name) {
      ...
      case 'civicrm_email':
        $op = "=";
        $query->_where[$grouping][] = "civicrm_email.is_primary $op 1";
        $query->_tables['civicrm_email'] = $query->_whereTables['civicrm_email'] = 1;
        return;
    }
  }
  ...
  public static function from($name, $mode, $side) {
    ...
    switch ($name) {
      ...
      case 'participant_email':
        $from = " LEFT JOIN civicrm_email ON (civicrm_participant.contact_id = civicrm_email.contact_id )";
        break;
    }
    return $from;
  }
